...
print_and_wait "No service-related anv variables above. Deleting the pod from the deployment causes the replicaSet to step in and recreate it:"
...
print_and_wait "Service exposing the deployment got created after the deployment. Recreating the pod now shows that it got populated with env variables with details about that service (e.g. WHISKER_PORT... and WHISKER_SERVICE...). As we saw this was not done automatically."
...
print_and_wait "As we can see the env file got updated without our intervention with the pods or deployment."
