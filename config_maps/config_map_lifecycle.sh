...
execute_command kubectl exec nginx-configmap --stdin --tty -- ls /etc/user-config/
execute_command kubectl exec nginx-configmap --stdin --tty -- cat /etc/user-config/editor.height
execute_command kubectl exec nginx-configmap --stdin --tty -- cat /etc/user-config/editor.width
execute_command kubectl exec nginx-configmap --stdin --tty -- cat /etc/user-config/store-name

print_and_wait "Now let's see that we have env variables available in our shell"
# TODO: execute_command wont parse pipe correctly
echo 'kubectl exec nginx-configmap --stdin --tty -- /bin/sh -c set | head -n 5'
kubectl exec nginx-configmap --stdin --tty -- /bin/sh -c set | head -n 5
echo
