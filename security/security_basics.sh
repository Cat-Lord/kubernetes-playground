...
echo 'kubectl get pods -o yaml | grep -P "serviceAccount(\w+)?: \w+"'
kubectl get pods -o yaml | grep -P "serviceAccount(\w+)?: \w+"
...
execute_command cat manual.secret.yaml
execute_command kubectl create -f manual.secret.yaml
...
kubectl describe secret/default-sa-token | grep uid | tr -s [:space:]
print_and_wait "This UID is the ID of the service account this secret is tied to"
# TODO: splitting commands with ';' doesnt' work properly with print_and_wait
echo 'kubectl get sa/default --template={{.metadata.uid}}; echo'
kubectl get sa/default --template={{.metadata.uid}}; echo
echo;
...
execute_command kubectl delete -f manual.secret.yaml
